---
source:
  - 'origin/260-Flex布局/01-flex簡介.md / 全文'
  - 'origin/260-Flex布局/02-flex布局.md / 全文'
---
...
- 現代 PC 端與移動端頁面都可以依版面需求使用 `flex` 彈性佈局。
- 只有在需要支援非常舊的瀏覽器時，才需要額外評估 `flex` 的兼容性與替代方案。
...
- **默認情況下**：`flex` 容器的寬度會由父容器的寬度決定。
- **默認情況下**：`flex` 容器的高度只會由子項目撐開。
...
- 初體驗例子中的 `span` 是子元素，也就是 `flex` 項目。
- `flex` 項目可以橫向排列也可以縱向排列。
