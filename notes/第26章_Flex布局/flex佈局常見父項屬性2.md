...
讀完本章後，你應該能用 Flexbox 的父項屬性處理常見的一維排版問題，而不是只背屬性名稱。

你應該能做到：

- 找出哪個元素應該設定 `display: flex`。
- 判斷 `flex-direction` 改變後，主軸與側軸在哪裡。
- 使用 `justify-content` 控制主軸上的排列與剩餘空間。
- 使用 `align-items` 控制單行在側軸上的對齊。
- 使用 `flex-wrap` 讓項目在空間不足時換行。
- 判斷多行情境下為什麼要使用 `align-content`。
- 使用 `flex-flow` 簡寫主軸方向與換行設定。

## 前置知識

本章需要先知道 HTML 的父子結構。Flexbox 的父項屬性不是寫在任意元素上就會控制整個畫面，而是寫在「包住一組子元素的父元素」上，用父元素安排它的直接子元素。

也需要知道基本盒模型。子元素的 `width`、`height`、`margin` 和父元素的可用空間，都會影響 Flexbox 排列後的畫面。
...
`display: flex` 會作為本章前提一起說明，因為父元素必須先成為 flex container，這些父項屬性才會以 Flexbox 的方式影響直接子元素。

來源範例中出現的 `align-self` 是子項屬性，本章只在必要時提醒，不深入展開。`flex-grow`、`flex-shrink`、`flex-basis`、`order` 等子項屬性也適合另開章節。

## 先看問題：盒子為什麼沒有照我想的方式排列？

假設畫面上有三個盒子。你想把它們排成一列、置中、平均分散，甚至在空間不足時換到下一行。初學者常直接問：「我要用哪個置中屬性？」但 Flexbox 的第一步不是背屬性，而是先看結構。
...
修改後，三個 `.box` 會成為 flex item，並依照 Flexbox 的預設規則排在同一列。這時先不用急著背完整屬性表，只要抓住一件事：父元素一旦設定 `display: flex`，它就開始負責安排直接子元素。

常見誤判是把 `display: flex` 寫在 `.box` 上，期待每個 `.box` 彼此被排好。那樣會讓每個 `.box` 自己變成 flex container，但不會讓三個 `.box` 被 `.box-list` 排列。要排列一組子元素，屬性要先回到它們共同的父元素上。
...
為什麼不是每個 `<a>` 變寬？因為 `justify-content` 主要處理的是項目之間和兩端的剩餘空間，不是直接改變每個項目的內容寬度。
...
這時很多人會繼續改 `align-items`，卻看不出想要的效果。原因是：你現在要控制的不是同一行內每個項目，而是多條 flex line 整體。
