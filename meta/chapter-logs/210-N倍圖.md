# 210-N倍圖

## 章節資訊

- origin：`origin/210-N倍圖/`
- 原始筆記：1 篇
- 原始資產：3 個
- 狀態總覽：`meta/chapter-status.md`

## 流程紀錄

### 2026-06-30

- 使用者確認 `origin/210-N倍圖` 原始資料已人工整理完成。
- 已同步 `meta/chapter-status.md`：`origin 整理` 標記為 `已完成`。
- 後續流程尚未執行，包含資產命名、alt 與連結文字整理、atomic 切分提案與下游產出。
- 使用 `asset-standardization` 以 apply 模式處理本地資產標準化命名。
- 已將 `origin/210-N倍圖/assets/images/` 內 3 個可唯一匹配圖片重新命名為 `n-times-image-img-###-hash6` 格式：`n-times-image-img-001-da3906.png`、`n-times-image-img-002-702660.jpg`、`n-times-image-img-003-e2d385.jpg`。
- 已同步更新 `origin/210-N倍圖/01-N倍圖.md` 中 3 個本地圖片引用為 `./assets/images/...` 標準路徑。
- `hash6` 皆由實體檔案 bytes 的 SHA-256 前 6 碼計算。
- 未發現無法匹配資產、需人工確認項目、重名衝突或跨文件共用資產。
- 已同步 `meta/chapter-status.md`：`資產命名` 標記為 `已完成`，完成率更新為 `14%`，下一步更新為 `alt/連結文字檢查`。

### 2026-07-01

- 已執行本地資產 alt 與連結文字整理（apply）。
- 已將 `origin/210-N倍圖/01-N倍圖.md` 的 Markdown 圖片 alt 從檔名式文字 `視網膜屏幕.png` 改為 `標準螢幕與 Retina 像素密度對比`。
- 已依 fenced code block 例外規則，替可唯一對應到 `assets/` 的兩個 HTML `img` 分別補上 `alt="綠底白色 Apple 標誌原圖"` 與 `alt="綠底白色 Apple 標誌二倍圖"`。
- 全章未發現需要整理的本地附件連結文字。
- 已同步 `meta/chapter-status.md`：`alt 與連結文字` 標記為 `已完成`，下一步改為 `atomic 切分提案`。

- 已完成 atomic 切分提案與 atomic notes 產生。
- 已寫入 `atomic/210-N倍圖/`，共 3 篇 atomic notes。
- 已同步 `meta/chapter-status.md`：`atomic 切分提案` 與 `atomic 產生` 標記為 `已完成`，完成率更新為 `36%`，下一步更新為 `atomic 內容審查`。

### 2026-07-01（Atomic 內容審查）

- 使用者已完成 `atomic/210-N倍圖/` Atomic 內容審查，審查範圍共 3 篇 atomic notes。
- 本次同步只更新 `meta/chapter-status.md` 與章節 log，未修改 `atomic/` 或 `origin/` 內容。
- 已同步 `meta/chapter-status.md`：`atomic 內容審查` 標記為 `已完成`，完成率更新為 `43%`，下一步更新為 `notes 生成`。

### 2026-07-02（Notes 生成）

- 已確認 `notes/210-N倍圖/` 已有正式教學筆記，共 2 篇 notes。
- 本次依已完成的 atomic 轉 notes 結果進行狀態同步，只更新 `meta/chapter-status.md` 與章節 log，未修改 `atomic/`、`origin/` 或 `notes/` 內容。
- 已同步 `meta/chapter-status.md`：`notes 生成` 標記為 `已完成`，完成率更新為 `50%`，下一步更新為 `notes 完成檢查`。

### 2026-07-02（Notes 完成檢查）

- 使用者已完成 `notes/210-N倍圖/` 正式教學筆記內容審查，審查範圍共 2 篇 notes。
- 本次同步只更新 `meta/chapter-status.md` 與章節 log，未修改 `origin/`、`atomic/` 或 `notes/` 內容。
- 已同步 `meta/chapter-status.md`：`notes 完成檢查` 標記為 `已完成`，完成率更新為 `57%`，下一步更新為 `notes 索引元資料`。

### 2026-07-02（Notes 索引元資料）

- 已完成 `notes/210-N倍圖/` 索引元資料標記，標記範圍共 2 篇 notes。
- 本次同步只更新 `meta/chapter-status.md` 與章節 log，未修改 `origin/`、`atomic/` 或 `notes/` 內容。
- 已同步 `meta/chapter-status.md`：`notes 索引元資料` 標記為 `已完成`，完成率更新為 `64%`，下一步更新為 `demos 生成`。

### 2026-07-02（Demos 生成）

- 已完成 `demos/210-N倍圖/` 教學 demo 生成，共 2 組 demo。
- 已建立 `01-邏輯像素、物理像素與設備像素比/`，展示 CSS 顯示尺寸、DPR 與圖片實際像素的換算關係。
- 已建立 `02-二倍圖的製作與顯示方式/`，對照原圖、正確使用二倍圖與未設定 CSS 尺寸的常見錯誤。
- Demo 皆包含固定格式 `Source notes` 註解，並複製所需圖片到各 demo 內的 `assets/`，維持自包含。
- 已完成靜態檢查、JS 語法檢查與資產尺寸檢查；瀏覽器驗證因內建瀏覽器安全政策拒絕開啟本機 `file://` 目標而未執行。
- 已同步 `meta/chapter-status.md`：`demos 生成` 標記為 `已完成`，完成率更新為 `71%`，下一步更新為 `practice 生成`。

## 下一步
- 進入 practice 生成。
