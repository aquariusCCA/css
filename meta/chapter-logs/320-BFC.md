...
- 使用者已完成 `notes/320-BFC/` 正式教學筆記內容審查，審查範圍共 2 篇 notes。
...
- 已完成 `notes/320-BFC/` 索引元資料標記，標記範圍共 2 篇 notes。
- 本次同步只更新 `meta/chapter-status.md` 與章節 log，未修改 `origin/`、`atomic/` 或 `notes/` 內容。
- 已同步 `meta/chapter-status.md`：`notes 索引元資料` 標記為 `已完成`，完成率更新為 `64%`，下一步更新為 `demos 生成`。

### 2026-07-02（Demos 生成）

- 已完成 `demos/320-BFC/` CSS 教學 demo 生成，共 2 組 demo。
- 已建立 `demos/320-BFC/01-BFC概念與建立條件/`，對應 `notes/320-BFC/01-BFC概念與建立條件.md`。
- 已建立 `demos/320-BFC/02-BFC常見用途與案例/`，對應 `notes/320-BFC/02-BFC常見用途與案例.md`。
- 已完成靜態檢查與 headless Chrome 瀏覽器驗證；兩組 demo 皆可載入，核心互動切換正常。
- 本次同步只更新 `meta/chapter-status.md` 與章節 log，未修改 `origin/`、`atomic/` 或 `notes/` 內容。
