...
`base-slug` 必須由任務明確提供，並作為命名基礎。不得自行推導、翻譯、改寫或用其他來源替代。

若 `base-slug` 缺少、空白、格式不符合英文 kebab-case，或無法判斷為英文語意 slug，必須列為「需要人工確認」，不得改寫 Markdown。

沒有明確語意的原始檔名不得沿用為 `base-slug`。
...
若提供的 `base-slug` 或原始檔名無法判斷為英文語意，必須列為「需要人工確認」，不得自行產生替代名稱。
