...
- 任務指定的 `base-slug`：英文語意 kebab-case slug。
...
2. 檢查任務指定的 `base-slug` 是否存在、非空白，且符合英文 kebab-case：`^[a-z0-9]+(-[a-z0-9]+)*$`。
3. 確認 Markdown 位於章節根目錄，不在章節底下的子目錄。
4. 檢查同章節是否有 `assets/` 目錄。
5. 不要在輸入檢查階段要求所有資產類型資料夾都存在。初步掃描指定 Markdown 的本地資產引用後，只針對實際引用的資產類型，檢查其對應目標資料夾：
   ...
   未被本次 Markdown 引用的資產類型資料夾缺失，不應阻止其他已可安全處理的資產重新命名與改寫。
6. 若必要輸入不足、`base-slug` 空白或格式不符合英文 kebab-case、Markdown 無法讀取、章節 `assets/` 目錄不存在，或 Markdown 不在章節根目錄，先列為「需要人工確認」，不要改寫 Markdown。
7. 若任務提供的章節目錄與 Markdown 實際位置不一致，先列為「需要人工確認」，不要猜測。
...
3. 在同章節的對應資料夾中尋找檔案。
   - 若該引用對應的目標資料夾不存在，將該引用列為「需要人工確認」，但不影響其他引用繼續處理。
4. 依序比對下列 basename：
...
- 找到多個可能的實體檔案。
- 實體檔案位於錯誤資產類型資料夾。
- 缺少、空白或格式不符合英文 kebab-case 的 `base-slug`。
- 無法讀取檔案內容或無法計算 hash。
- 目標檔名衝突且無法安全判斷是否同內容。
