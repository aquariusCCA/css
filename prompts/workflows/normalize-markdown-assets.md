...
3. 檢查同章節是否有 `assets/` 目錄。
4. 不要在輸入檢查階段要求所有資產類型資料夾都存在。初步掃描指定 Markdown 的本地資產引用後，只針對實際引用的資產類型，檢查其對應目標資料夾：
...
5. 若必要輸入不足、Markdown 無法讀取、章節 `assets/` 目錄不存在，或 Markdown 不在章節根目錄，先列為「需要人工確認」，不要改寫 Markdown。
6. 若任務提供的章節目錄與 Markdown 實際位置不一致，先列為「需要人工確認」，不要猜測。
