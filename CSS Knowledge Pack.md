本 CSS 筆記包用來管理 CSS 學習資料、原子化資料、正式筆記、附錄查表資料、補充內容、示範程式、練習題、複習材料與 AI 提示詞。
...
| `appendix/` | 附錄資料區 | 根據 `notes/` 生成查表型資料，例如選擇器、屬性、值、單位、函式、布局模式或瀏覽器支援注意事項等。 |
| `demos/` | 示範程式區 | 根據 `notes/` 生成可執行或可參考的 CSS 範例。每組 demo 對應單一筆記，聚焦一個 CSS 知識點。 |
| `projects/` | 整合專案區 | 根據跨章節的多篇 `notes/` 生成可獨立開啟執行的 CSS 整合練習或小型作品。 |
...
* `atomic review` 是進入正式筆記前的審查關卡，確認 CSS 技術正確性、概念歸屬、教學順序與範例可用性。
...
* 正式筆記應清楚區分 CSS 概念、語法、效果、限制、常見錯誤與實作情境。
...
* CSS demo 可使用必要的頁面結構作為渲染載體，但教學重點應放在 CSS 寫法、效果與行為觀察。
...
* 專案只能組裝來源 notes 已教過的 CSS 技術，不應引入尚未建立筆記來源的新知識。
...
* 目前章節數量與章節順序仍待依 CSS 課程架構重新確認，不沿用原筆記包章節數量。
