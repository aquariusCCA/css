...
本 skill 負責 `projects/` 跨章節整合層。它解決一個結構痛點：`demos/` 以單篇 notes 為單位（`demos/<章節>/<筆記名>/`，1:1 微觀範例），因此「結合多章 CSS 知識的完整頁面（例如同時用到選擇器、盒模型、排版、色彩、Flexbox/Grid、定位與響應式設計的產品列表頁）」在 `demos/` 中無處可放。`projects/` 就是這類**宏觀綜合場景**的家。
...
`projects/` 是**選用層**，不在單章節資料流之內；只有當「把多章 CSS 知識組成一個真實頁面」的教學價值明確高於各自獨立的 demo 時才生成。寧可不建，不過度生成。
...
啟用本 skill 時，扮演「前端整合專案設計師」與「CSS/HTML/JavaScript 教學實作審查員」，目標是把**多篇已完成的 CSS notes** 收斂成一個可直接打開、有完整情境的綜合頁面，讓讀者看到「分散學到的 CSS 知識點如何在一個真實頁面裡協作」。
...
能從整體頁面回推各個 CSS 選擇器、宣告、布局規則與響應式規則如何分工
```
...
> 專案只能組裝 `source_notes` 清單所列各篇 notes **已教過 CSS 技術的聯集**，不得引入任何一篇來源 notes 都未涵蓋的新 CSS 知識。

這是把單親追溯（demos 的 1:1）泛化成多親追溯（projects 的多對一）後，仍要守住的「下游不得超出來源」原則。判斷時：

1. 專案用到的每一項 CSS 選擇器、宣告、屬性值、布局模式、定位方式、視覺效果、響應式規則與互動狀態，都必須能在 `source_notes` 其中至少一篇找到支撐。
2. 若某個必要技術沒有任何來源 notes 教過，有兩個選項：將對應的 notes 補進 `source_notes`（且該章節須已就緒），或把該技術從專案中移除。兩者皆不可行時，列入「需要人工確認」，不要自行引入。
3. HTML 只作為必要頁面載體，應保持簡潔語意，不得成為夾帶未教 HTML 主題的破口；JS 僅用於輔助觀察或切換狀態，不取代 CSS 本身要展示的效果。
...
```text
專案主題或目標：例如「響應式產品卡片牆」「儀表板版面」「部落格文章排版」
...
1. 專案必須有一個清楚、真實的頁面情境（一個卡片牆、一個儀表板、一個文章頁、一個列表頁…），不是知識點的拼盤。
...
1. 採全域 `<編號>-<專案標題>`，編號三位數、間隔 10（如 `010-響應式產品卡片牆`、`020-儀表板版面`），與章節編號各自獨立。
...
source_notes:
  ...
  - notes/080-盒模型/01-盒模型尺寸計算.md
  - notes/120-Flexbox/02-flex容器屬性.md
  - notes/150-響應式設計/01-media-query基礎.md
topics: [選擇器, 盒模型, Flexbox, 響應式]
summary: 綜合選擇器、盒模型、Flexbox 與 media query，組成一個響應式產品卡片牆。
prerequisites: [040-選擇器, 080-盒模型, 120-Flexbox, 150-響應式設計]
level: 綜合
---
...
3. 必須讀取候選 notes 全文，確認它們實際教過專案需要的 CSS 選擇器、屬性、屬性值、布局、狀態或響應式規則。
...
```text
專案：projects/010-響應式產品卡片牆/
頁面情境：一個產品列表頁，卡片在桌機以多欄 Flexbox 排列，窄螢幕改為單欄，並呈現一致的間距、邊框與文字層級。
各知識點作用：
  040 選擇器 → 以 class selector 與 cascade 控制卡片狀態與文字層級
  080 盒模型 → 控制卡片 padding、border、gap 與尺寸
  120 Flexbox → 排列卡片列表與卡片內部資訊
  150 響應式設計 → 用 media query 調整欄數與間距
整合紅線：頁面只用到上述四篇 notes 教過的技術，未引入新知識。
prerequisites：040-選擇器、080-盒模型、120-Flexbox、150-響應式設計（皆須「notes 索引元資料」已完成）
```

### 範例二：不生成（單篇即可展示）

```text
來源 notes：notes/080-盒模型/02-display與盒型.md
```

判斷：單篇 notes 即可在一組 demo 中展示 `display` 各值的盒型差異，沒有跨章節綜合需求。

```text
原因：屬單一 CSS 知識點，宜回到 notes-demo-generation 在 demos/080-盒模型/ 生成單章 demo，不需整合專案。
```

### 範例三：不生成（來源章節未就緒）

```text
來源 notes 涉及 120-Flexbox、150-響應式設計
但 150 的「notes 索引元資料」尚為「進行中」
```

判斷：生成閘門未通過。

```text
原因：150-響應式設計 尚未完成 notes 索引元資料；先完成該章前置，再回來生成整合專案。
```
